...
internal fun RepoItemsSearchResponse.toRepoItemsList(colors: Map<String, ArgbColor>): List<RepoItem> = items
  ?.map { it.toRepoItem(colors) }
  ?: emptyList()

private fun RepoItemsSearchResponse.Item.toRepoItem(colors: Map<String, ArgbColor>): RepoItem = RepoItem(
  id = id,
  fullName = fullName,
  language = language,
